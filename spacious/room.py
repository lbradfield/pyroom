...
class Polygon(object):
    ...
    def __init__(self, points):
        ...
        config.logger.debug("constructing with points: " "%s", points)
    ...
    def rotate(self, angle):
        ...
        for vertex in self.vertices:
            rotated_vertices.append(self.rotate_point(vertex, mp_angle, self.centroid))
    ...
    def calc_centroid(vertices, s_area):
        ...
        """
        config.logger.debug("vertices: %s, s_area: %s", vertices, s_area)
        c_x = mp.mpf()
        c_y = mp.mpf()
        num_vertices = len(vertices)
        # high-precision calculation
    ...
    def rotate_point(point, angle, origin):
        ...
        calculations.
        """
        config.logger.debug("point=%s, angle=%s, origin=%s", point, angle, origin)
        sin_ang = sin(angle)
        cos_ang = cos(angle)
        o_x = origin[0]
        o_y = origin[1]
        ...
        config.logger.debug("rotated point: %s, %s", new_x, new_y)
        return new_x, new_y
