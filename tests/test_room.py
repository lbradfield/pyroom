...
class PolygonTestCase(unittest.TestCase):
    ...
    def test_area_signed(self):
        self.assertEqual(self.polygon1.calc_area(self.polygon1.vertices), (A1, SA1))
        self.assertEqual(self.polygon2.calc_area(self.polygon2.vertices), (A2, SA2))
        self.assertEqual(self.polygon3.calc_area(self.polygon3.vertices), (A3, SA3))
        ...
        for i, angle in enumerate(ROTATE_ANGLES):
            with self.subTest(angle=angle):
                for j in range(len(V1)):
                    self.assertEqual(
                        Polygon.rotate_point(V1[j], angle, C1), P1_ROTATED[j]
                    )
                for j in range(len(V2)):
                    self.assertEqual(
                        Polygon.rotate_point(V2[j], angle, C2), P1_ROTATED[j]
                    )
                for j in range(len(V3)):
                    self.assertEqual(
                        Polygon.rotate_point(V3[j], angle, C3), P1_ROTATED[j]
                    )
    ...
    def test_rotate(self):
        for i, angle in enumerate(ROTATE_ANGLES):
            with self.subTest(angle=angle):
                ...
                self.assertEqual(self.polygon1.vertices, P1_ROTATED[i])
                self.assertEqual(self.polygon2.vertices, P2_ROTATED[i])
                self.assertEqual(self.polygon3.vertices, P3_ROTATED[i])
            self.polygon1.rotate(-angle)
            self.polygon2.rotate(-angle)
            self.polygon3.rotate(-angle)
            self.assertEqual(self.polygon1.vertices, V1)
